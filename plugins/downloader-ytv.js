...
let handler = async(m, { conn, args, isPrems, isOwner }) => {
    if (!args || !args[0]) return conn.reply(m.chat, 'Ähm... wo ist die URL?', m)
    let chat = global.db.data.chats[m.chat]
    let server = (args[1] || servers[0]).toLowerCase()
    let { dl_link, thumb, title, filesize, filesizeF } = await ytv(args[0], servers.includes(server) ? server : servers[0])
    let isLimit = (isPrems || isOwner ? 99 : limit) * 1024 < filesize
    conn.reply(isLimit ? `Dateigröße: ${filesizeF}\nDateigröße über ${limit} MB, eigener Download: ${dl_link}` : global.wait, m)
    let _thumb = {}
    try { _thumb = { thumbnail: await (await fetch(thumb)).buffer() } } catch (e) {}
    m.reply(wait)
    if (!isLimit) await conn.sendButtonVid(m.chat, dl_link, `*Titel:* ${title}\n*Dateigröße:* ${filesizeF}`.trim(), wm, 'menu', '.?', m)
