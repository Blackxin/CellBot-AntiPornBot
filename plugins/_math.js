...
handler.before = async function (m) {
  ...
  try {
    if (!(id in this.math) && /^apa hasil dari/i.test(m.quoted.text)) return m.reply('Die sache ist erledigt')
    ...
    if (m.text == math.result) {
      ...
      if (--this.math[id][2] == 0) {
        clearTimeout(this.math[id][3])
        delete this.math[id]
        m.reply(`*Gelegenheit läuft aus!*\nAntwort *${math.result}*`)
      } else m.reply(`*falsche Antwort!*\nEs gibt noch ${this.math[id][2]} Chancen`)
    }
    // }
    return !0
  } catch (e) {
